...
public class KHG_Line : MonoBehaviour
{
    ...
    public void SetLine()
    {
        ...
        {
            vec = new Vector3(pos.x + (i - 205) * (-4.87f - pos.x) / 19f, pos.y + (i - 205) * (1.52f - pos.y) / 19f, pos.z + (i - 205) * (2.476f - pos.z) / 19f);
            line.SetPosition(i, vec);
        }
    ...
    IEnumerator SetLineColor()
    {
        ...
        GradientColorKey[] gColor = new GradientColorKey[] { new GradientColorKey(origin, 1.0f), new GradientColorKey(origin, 1.0f) };
        ...
        if (status == 1)
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                gColor[0].time = 1f - 0.01f * (i + 1); gColor[1].time = 1.0f;
                gradient.SetKeys(gColor, gAlpha);
                line.colorGradient = gradient;
                yield return ws;
            }
